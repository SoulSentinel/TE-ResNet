...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        features = self.processor.extract_lps(audio)
...
def train(model, train_loader, criterion, optimizer, device):
    ...
    for inputs, labels in tqdm(train_loader, desc="Training LPS"):
...
def evaluate(model, data_loader, criterion, device, evaluator):
    ...
    with torch.no_grad():
        for inputs, labels in tqdm(data_loader, desc="Evaluating LPS"):
            inputs, labels = inputs.to(device), labels.to(device)
...
def main():
    ...
    for epoch in range(num_epochs):
        ...
        if eval_eer < best_eval_eer:
            ...
            torch.save(model.state_dict(), 'best_model_lps.pth')

    # Close the TensorBoard writer
    writer.close()

    # Load the best model and perform final evaluation
    model.load_state_dict(torch.load('best_model_lps.pth'))
